...
describe("standard prompt", () => {
  it("contains the required bilingual sprite-sheet production contract", () => {
    ...
    expect(prompt).toContain("clean 5×3 transparent PNG sprite sheet");
    ...
    expect(prompt).toContain("true transparent RGBA background");
    expect(prompt).toContain("Row order: 1–5 standard select");
    expect(prompt).toContain("建立一個乾淨的 5×3 透明 PNG 精靈圖");
